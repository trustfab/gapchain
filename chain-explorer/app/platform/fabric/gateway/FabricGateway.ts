...
export class FabricGateway {
		...
		try {
			...
			if (mTlsIdLabel) {
				...
				if (this.clientTlsIdentity !== undefined) {
					...
					// Create TLS identity from certificate files
					logger.info('Creating TLS identity from certificate files');
					await this.enrollTlsIdentity(mTlsIdLabel);
					connectionOptions.clientTlsIdentity = mTlsIdLabel;
				}
			}
	 ...
	 *
	 */
	async enrollTlsIdentity(tlsIdentityLabel) {
		try {
			// eslint-disable-next-line @typescript-eslint/no-var-requires
			const fs = require('fs');

			// Read TLS certificate and key from files
			const tlsCerts = this.fabricConfig.getTlsCerts();
			const tlsCertPath = tlsCerts.client.cert.path;
			const tlsKeyPath = tlsCerts.client.key.path;

			logger.info('TLS identity tlsCertPath:', tlsIdentityLabel);
			logger.info('TLS identity tlsCertPath:', tlsIdentityLabel);
			const tlsCertPem = fs.readFileSync(tlsCertPath, 'utf8');
			const tlsKeyPem = fs.readFileSync(tlsKeyPath, 'utf8');

			const tlsIdentity = {
				credentials: {
					certificate: tlsCertPem,
					privateKey: tlsKeyPem
				},
				mspId: this.fabricConfig.getMspId(),
				type: 'X.509'
			};

			logger.info('Enrolling TLS identity:', tlsIdentityLabel);
			await this.wallet.put(tlsIdentityLabel, tlsIdentity);
			return tlsIdentity;
		} catch (error) {
			logger.error('Error enrolling TLS identity:', error);
			throw new ExplorerError(
				`Failed to enroll TLS identity: ${error instanceof Error ? error.message : String(error)
				}`
			);
		}
	}
	...
	async queryInstantiatedChaincodes(channelName) {
		...
		try {
			...
			logger.warn('Failed to query lscc, fallback to _lifecycle');
		}
